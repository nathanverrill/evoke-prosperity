...
**Implementation note:** this table only proposes the `mission_quest` pairing — actual block palettes, NPC placements, and datapack hooks belong to whoever builds the world content in `evoke-infra/minecraft/world-seed/`, referencing the existing `halyard_rent_functions`/`custom_drops` datapacks already migrated per `BUILD_PLAN.md`.
