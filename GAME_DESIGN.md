...
**Implementation note:** `primary_skill`/`secondary_skill` are already synced from the LMS into Postgres's `missions` table (`main.py:95-105`, `init-db.sql:81-82`) — the data pipeline exists, it's just never read at award time today. The scoped build work is: (a) read both columns instead of just `superpower` when publishing `BadgeAwarded` after a mission completion, (b) apply the alias table above, (c) carry the resolved Power key(s) — and their *true* Quality, not the mission's `Superpower` field — in the event payload, (d) extend the `player-profile` projection to track per-Power `earned` state (not just per-Quality), deriving Quality-badge completion from "all 4 child Powers earned," and (e) build the two non-mission triggers that already have a real event to hang off (Generosity of Spirit off `InsightPublished(kind="peer")`, Curiosity off billbot chat) before the two that depend on unbuilt §7.2/§7.3 features.
...
- **PolyFactory mod** (pipe/rotation/energy systems in `true_oasis/data/polyfactory/`) — a genuine factory-automation system is installed in the world, which is good news: it validates the "Factory Crafting" instinct below even though the specific narrative framing here was a guess.

**This table below is therefore a proposed narrative framing to lay over what exists, not a build spec** — treat every row as a hypothesis to check against the real world content (NPCs, structures, command blocks in `true_oasis`) rather than as new work. A full reconciliation pass by whoever has hands-on access to the built world is still needed; this session only sampled a few files.
...
**Implementation note:** since the world and its mechanics already exist, the work here isn't building new minigames — it's confirming (or correcting) which `mission_quest` each existing system should be tagged as, and wiring `QuestCompleted` events to the real triggers (e.g. the rent timer clearing, the lift precheck passing) instead of inventing new ones. Start from the confirmed mechanics above (rent timer → M5/M6, lift precheck → the Halyard-access gate before M5) and correct the rest of the table against `true_oasis`'s actual NPCs/structures/command blocks before treating any row as final.
