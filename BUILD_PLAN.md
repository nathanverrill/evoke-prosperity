...
5. ❌ **Gamified web app** — *not done.* `evoke/static/index.html` is still the original pipeline-demo page (a "Dev Login" button, a bare missions list, a notifications list) — not the mission-loop SPA specified in `UI_SPEC.md`. The profile APIs it needs now exist (#4), so this is no longer blocked on anything else in this list.
6. ❌ **End-to-end pass** — blocked on #3 and #5.
