...
evoke-prosperity-files/minecraft/
  minecraft-world-files/basin/      the custom Basin Simulation world (command
                                          blocks, scripts, embedded datapacks)
  minecraft-world-files/unmodded/         unmodded fallback variant of basin
...
The B1llbot chat mod being Fabric dictates the server flavor, and it supersedes `ARCHITECTURE.md`'s "no custom plugin, log-tail only" decision. **Status update, since this was written:** the server was upgraded to Minecraft 26.2, and the mod's compiled bytecode is remapped against the *old* Minecraft version's intermediary mappings — it fails at runtime on 26.2 (`NoClassDefFoundError`), and Fabric hasn't published Yarn mappings for 26.2 yet to rebuild it against. The mod is currently **not loaded** in the shipped image; see `evoke-infra/minecraft/Dockerfile`'s own comment for the full story and the re-add condition. B1llbot's *web-facing* chat (Hub drawer, Companion Mode, `#/billbot`) is unaffected — that's a separate integration through OpenWebUI, see `GAPS.md`.

Container shape — replace `image: itzg/minecraft-server` with `build: ./minecraft` in `evoke-infra/`:

```
evoke-infra/minecraft/
  Dockerfile          # FROM eclipse-temurin:25-jre (bumped from :21-jre -- Minecraft
                      # 26.2's server jar needs Java 25); pinned Minecraft + Fabric
  ...
  mods/               # Geyser-Fabric, Floodgate, Fabric API -- version-pinned.
                      # billbot-1.0.1.jar is present but NOT loaded (see above);
                      # the Dockerfile's patch step for it is disabled, not removed.
  world-seed/         # basin, the real Basin Simulation world -- gitignored
                      # (704MB), placed manually before building; see its own
                      # README.md. Loads automatically once present; server falls
                      # back to vanilla generation if this directory is empty.
...
Done means, on localhost, with real events on Redpanda. **Status per item, checked against what's actually been run (not assumed) as of build steps 1–5:**

1. ✅ Both stacks come up healthy from scratch — verified for `web`/`brightspace-sim`/`minecraft-bridge` (step 1) and the Minecraft container (step 2) individually; not yet verified as one single `docker compose up -d` twice with zero manual steps end-to-end.
2. ✅ Missions visible in EVOKE are the ones pulled from `brightspace-sim`'s assignment store — verified (step 3), though the "delete one, refresh sync, it disappears" direction specifically (removal, not just add/update) hasn't been exercised.
3. ⚠️ **Partially verified.** graphic-novel panel → mission brief → evidence submit → common award: ✅ verified with a real browser (step 5). AI-pass epic award: code path exists (`trigger_ai_review`) but not exercised against a real Ollama/OpenWebUI — no AI backend was available to test against. Teacher-grades-in-sim → third award: ✅ verified (step 3, via direct API calls rather than the sim's own review-page UI).
4. ⚠️ **Partially verified.** Collect UI flow and the resulting `RewardCollected` event: ✅ verified (steps 4–5). Actual RCON delivery into a *running* Minecraft container: not tested — the Minecraft container (step 2) and the app+bridge stack (steps 3–5) have not yet been run together in one integration pass.
5. ✅ Verified (step 5, Companion Mode) — self-report landed as a real `mc_quest_completions` row, confirmed directly in Postgres. Screenshot upload specifically wasn't exercised (observation text only).
6. ✅ Verified (steps 4–5) for player profile; team profile verified via direct API calls (step 4), not re-driven through the SPA UI in the same pass.
7. ❌ Not tested — kill/restart mid-flow idempotency.
8. ✅ Verified — every test this build ran had `AI_ENABLED=false` and still produced the common + teacher-review award path.
9. ❌ Not tested — no OpenWebUI instance was available; B1llbot's chat UI works and calls the right endpoint (verified), but there's nothing configured on the other end to answer in character. Matches GAPS.md's existing "OpenWebUI persona setup is manual" gap.
10. ❌ Not tested — no real Bedrock/Java Minecraft clients were used; the Minecraft container itself was verified running (step 2) but not joined by actual game clients.
...
2. ✅ **Custom Minecraft container** — replaces `itzg/minecraft-server`, built and verified end-to-end (not just written): image builds, server boots, all mods load, all four datapacks load with no errors, real RCON auth + `list` command both succeeded against the running server. Found and fixed two real version-compatibility bugs by actually running it: Fabric Loader needed bumping from the mod's own 0.16.10 pin to 0.19.3 (current Fabric API/Geyser-Fabric builds require >=0.17.x; safe since the mod doesn't hard-pin an exact loader version), and two of the four datapacks silently failed to load on a `pack_format: 88` declaration missing fields MC 1.21.10 then required above format 81 — lowered to `pack_format: 48` to match the two datapacks that already worked.
   **Update (later pass):** upgraded to Minecraft 26.2 (Java bumped 21→25 to match). The real `basin` world — previously deliberately not done, falling back to vanilla generation — is now loaded (manual placement, per `world-seed/README.md`'s own options; still gitignored, not committed). BillBot's in-game mod is currently **not loaded** — it fails at runtime on 26.2 (old intermediary mappings baked into its compiled bytecode) and can't yet be rebuilt (no Yarn mappings published for 26.2 as of this note). See `evoke-infra/minecraft/Dockerfile`'s comment for the full story; B1llbot's web-facing chat is a separate, working integration through OpenWebUI.
3. ✅ **Missions-from-sim inversion** — `brightspace-sim` self-seeds the real 12 missions as Brightspace-shaped assignments (`GET /d2l/api/lp/1.96/dropbox/assignments`); EVOKE's `sync_missions_from_lms()` pulls and upserts them into `missions` on startup, keyed by a new `UNIQUE(campaign_id, lms_assignment_ref)`. `seed.py` no longer seeds missions. The teacher-grading webhook round trip (sim → EVOKE) is now wired for the first time — it never fired before. Verified end-to-end: real sync, a real submission → grade → webhook → award chain resolving both `user_id` and `mission_id` correctly. Found and fixed 4 more previously-undiscovered bugs in the process (hardcoded `"m1"` dropbox target, a JSON-vs-form-encoding mismatch, `seed.py` unable to complete a single run for lack of `psycopg2.extras.register_uuid()`, and quest-to-mission linking that has never worked due to a key mismatch) — see the commit for detail.
4. ✅ **Event catalog completion** — `MissionCompleted`/`XPGranted`/`BadgeAwarded` now fire in `/api/submit-evidence`; `QuestCompleted`/`XPGranted` now fire in `/api/mc-quests/{quest_id}/submit` (deduped so a resubmission doesn't double-grant XP). New `player-profile`/`team-profile` OpenSearch projections (built by a new PROFILE WORKER branch in `workers.py`) and their read APIs (`GET /api/profile/player/{user_id}`, `GET /api/profile/team/{team_id}`). Verified end-to-end against a real Postgres+Redpanda+OpenSearch+MinIO stack — submitted evidence, watched the events process, confirmed real OpenSearch documents, called both read APIs, confirmed correct XP/badge/mission/quest aggregation and correct resubmission dedup. **Found and fixed a real pre-existing bug in the process:** `EvidenceSubmitted` has always been published with a `user_id` field, but the AI Coach worker read `learner_id` — silently crashing the entire worker loop (one big try/except, no per-message isolation) on the very first event, meaning the AI Coach worker, the Search/Timeline worker, and this new Profile worker had never actually run in this codebase's history. Fixed the field name and added per-message error isolation so one bad event can no longer take down every consumer permanently. XP values (100/mission, 40/quest) are explicit placeholders pending GAPS.md's still-undecided XP economy; badge-earning criteria (every tagged mission vs. a threshold) is likewise a placeholder pending GAPS.md's still-undecided badge criteria.
5. ✅ **Gamified web app** — `evoke/static/index.html` replaced with the mission-loop SPA per `UI_SPEC.md` (Operations Hub, novel reader, mission brief/debrief, player/team profile), wireframe tokens.css/layout.css/theme.css split, restyled Companion Mode. Verified with a real headless-browser run (Playwright, not just code review): full mission loop (novel → brief → submit → live timeline → debrief → collect), profile pages showing correct real data, novel chapter lock/panel navigation, B1llbot drawer, and a Companion Mode quest self-report confirmed landing in Postgres. Found and fixed two more bugs in the process: the step-1 Dockerfile fix silently broke static file serving (path was resolved relative to CWD, not the file), and the `learner-timeline` projection never actually initialized its step scaffold. Companion Mode's pre-existing Collect/B1llbot-chat calls (JSON body vs. the query params the endpoints actually read) were also broken until now. **Out of scope, called out inline rather than faked:** mission lock/gating (no backend enforcement exists yet), a "my team" lookup endpoint, and real graphic-novel art (a wireframe placeholder manifest stands in — see GAPS.md's content-pipeline gap).
6. ⚠️ **End-to-end pass — partially run, not complete.** See the itemized acceptance-scenario status above: 5 of 10 items fully verified, 3 partially verified, 2 not yet touched (idempotent-restart test, and running the Minecraft container + app stack together with real Bedrock/Java clients and a live OpenWebUI). No longer blocked on missing pieces — what's left is running the remaining scenarios, not building anything new.
